...
public class DrawCardState extends GameState {
  ...
  /**
   * Draws a card from the VisibleResourceCards list into playerToken's hand, based on the player's
   * choice
   *
   * @param playerToken Token that represents the player
  ...
  public boolean drawVisibleResourceCard(PlayerToken playerToken, int choice) {
    return gameFlowManager.getTurn().equals(playerToken)
        && gameModelUpdater.drawVisibleResourceCard(playerToken, choice);
  }
  ...
  public boolean drawVisibleGoldCard(PlayerToken playerToken, int choice) {
    return gameFlowManager.getTurn().equals(playerToken)
        && gameModelUpdater.drawVisibleGoldCard(playerToken, choice);
  }
}

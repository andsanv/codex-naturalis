...
public class RMIConnectionHandler extends ConnectionHandler {
    ...
    private final BlockingQueue<ServerCommand> serverCommandQueue;
    ...
    public RMIConnectionHandler() {
        try {
            ...
            new Thread(new CommandConsumer<ServerCommand>(serverCommandQueue, this)).start();
    ...
    @Override
    public boolean sendToMainServer(ServerCommand serverCommand) {
        try {
            mainServerActions.send(serverCommand);
            return true;
        } catch (Exception e) {
            e.printStackTrace();
        }

...
public class InitializationState extends GameState {
    ...
    public boolean handleInitialization(Map<String, PlayerToken> idToToken, Map<PlayerToken, StarterCard> tokenToStarterCard, Map<PlayerToken, CardSide> tokenToCardSide, Map<PlayerToken, ObjectiveCard> tokenToObjectiveCard) {
        ...
        // add Player objects
        idToToken.values().forEach(playerToken -> gameModelUpdater.addPlayer(playerToken, tokenToStarterCard.get(playerToken), tokenToCardSide.get(playerToken), tokenToObjectiveCard.get(playerToken)));

        // fill players' hands
        idToToken.values().forEach(playerToken -> {
        ...
        gameModelUpdater.setCommonObjectives(new ArrayList<>(Arrays.asList(gameModelUpdater.drawObjectiveCard().get(), gameModelUpdater.drawObjectiveCard().get())));

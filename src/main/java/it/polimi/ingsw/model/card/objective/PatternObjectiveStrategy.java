...
/**
 * Strategy to calculate points for an objective card based on patterns on a
 * player board.
 */
public class PatternObjectiveStrategy implements ObjectiveStrategy {
    ...
    public int getCompletedOccurrences(PlayerBoard playerBoard) {
        Map<Coords, Resources> board = playerBoard.getBoard().entrySet().stream()
                .filter(e -> !e.getKey().equals(new Coords(0, 0)))
                .collect(Collectors.toMap(
                        Map.Entry::getKey,
                        e -> e.getValue().getType().get() // isPresent() isn't checked since the starter card has been
                                                          // filtered out.
                ));

        // Early return if only the starter card has been placed
        if (board.keySet().size() == 0)
            return 0;
